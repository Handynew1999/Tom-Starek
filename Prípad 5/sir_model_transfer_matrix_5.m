function [S1,I1,R1,S2,I2,R2,S3,I3,R3,R01,R02,R03,x1,x2,x3,y1,y2,y3,P1,P2,P3,P4,P5,P6,P7,P8,ti1,ti2,ti3,ti4,ti5,ti6,ti7,ti8,R021,R022] = sir_model_transfer_matrix_5(beta,gamma,N,I0,T,dt,delta)
    P=0;
    S1 = zeros(1,T/dt);
    S1(1) = (N(1)-I0(1))*(1-P);
    I1 = zeros(1,T/dt);
    ...
    for tt = 1:(T/dt)-1
        ...
        if I1(tt)/(S1(tt)+R1(tt))>=0.02 && opatrenie11==0
         opatrenie11=1;
         beta(1)=beta(1)*0.345; %alfa0=0.63 %koord=0.40 %nekoord=0.26 %èas=0.26
         P1=I1(tt);
         ti1=tt;
        end
        ...
        if I2(tt)/(S2(tt)+R2(tt))>=0.02 && opatrenie21==0
         opatrenie21=1;
         beta(2)=beta(2)*0.345; %alfa0=0.63 %koord=0.4 %nekoord=0.35 %èas=0.365
         P5=I2(tt);
         ti5=tt;
        end
